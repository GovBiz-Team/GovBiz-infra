...
모노레포에서 관리합니다. Django Ops도 그 저장소의 `backend/ops-service/`에서 개발하며,
...
| 서비스 경계 | Core·Catalog·AI·Ops를 구분. Catalog는 앱 저장소에서 독립 프로세스·DB와 Core HTTP 연동을 로컬 검증했고, Kubernetes·AWS 운영에는 아직 반영하지 않음 |
| 아직 미구현 | Argo CD 연결, Core·Catalog·AI의 Kubernetes 이전, Ops 관리자 인증·LLMOps 업무 기능, AWS Kubernetes 운영 전환 |
...
| 저장소 | 책임 |
| --- | --- |
| [GovBiz](https://github.com/GovBiz-Team/GovBiz) | React 웹·React Native 모바일·공통 패키지·Core·Catalog·AI·Ops 코드, 테스트, Dockerfile, 로컬 Compose |
| [GovBiz-infra](https://github.com/GovBiz-Team/GovBiz-infra) | 로컬 Kubernetes 배포 설정·검증, 향후 운영/GitOps 전환·복구 절차 |

React Native는 별도 `GovBiz-app` 저장소를 전제로 하지 않습니다. 현재 `GovBiz/mobile/`에 있으며,
...
- **Catalog Service:** 선택형 분리 모드에서 제공처 수집·정규화·검색 색인과 원본 DB를 소유합니다.
  Core는 인증된 HTTP snapshot으로 조회용 복제본을 유지합니다. [구현 범위·전환 조건](https://github.com/GovBiz-Team/GovBiz/blob/develop/docs/catalog-service-extraction.md)을 따릅니다.
...
GovBiz에서 고유 태그의 Ops 이미지를 빌드하고 이 저장소의 `scripts/smoke_kubernetes.py`를 실행합니다.
...
├─ docs/
│  ├─ service-boundaries.md   Core·Catalog·AI·Ops 책임·데이터·인증 계약
...
- Django Ops: `backend/ops-service/`
...
위 이름은 **소스 폴더명**입니다. 호환성을 위해 기존 Compose의 `core-api`·`django-api`,
ECR의 `govbiz/core-api`, 이 저장소의 Kubernetes 리소스 `operations-api`, DB·볼륨 이름은 유지합니다.
소스 폴더 변경을 이유로 기존 운영 리소스나 데이터를 새로 만들지 않습니다.
...
3. NetworkPolicy 집행과 probe·종료·단일 scheduler·상태 저장소 정책을 확인한 뒤 Core·Catalog·AI를 단계적으로 이전합니다.
